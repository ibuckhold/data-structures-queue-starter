...
class Queue {
    ...
    dequeue() {
        ...
        if(this.length === 1) { //Should reassign the front and back pointers to null if there is only one node in the queue
            this.front = null
            this.back = null
        } else { //if it's not 0 or 1, then is 2 or greater
        //Should reassign the front pointer to the node just behind the front node for queues of size two or greater
